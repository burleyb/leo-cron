...
const logger = leoLogger.sub("[cron/lib/utils]")

import lambda from "./targets/lambda.js";
import fargate from "./targets/fargate.js";
import sns from "./targets/sns.js";
import url from "./targets/url.js";
import zlib from 'zlib';

const invocationTargets = { lambda, fargate, sns, url };
...
let util = {
		...
		return async function(target, newImage, oldImage, payload) {
			...
			if (invocationTarget) {
				...
				try {
					...
					let dynamodbCommand = new UpdateCommand(command);
					let saveValue = await dynamodb.docClient.send(dynamodbCommand);
					...
					for (var k = 0; k < iids.length; k++) {
						...
						try {
							...
							logger.log("===Success===", cron.id, payload.__cron.iid, JSON.stringify(data, null, 2));
						} catch (err) {
							try {
								logger.log("===Error===", `${cron.id} - ${cron.name} - ${payload.__cron.iid} `, err);
								let command = {
									TableName: options.tableName,
									Key: {
										id: cron.id
									},
									UpdateExpression: 'set #endTime = :endTime, #status = :status, #log = :log',
								...
								if (typeof options.releaseLockOnError == "function") {
									command = options.releaseLockOnError(command, newImage, oldImage, payload);
								}
								
								let dynamodbCommand = new UpdateCommand(command);
								await dynamodb.docClient.send(dynamodbCommand);
							} catch (error) {
	...
	backoff: (options) => {
		...
		return async function(newImage, oldImage, backoffFunction) {
			...
			return await dynamodb.docClient.send(command);
		}
	},
