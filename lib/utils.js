...
let util = module.exports = {
	...
	shouldRun: async function(newImage, oldImage) {
		...
		let progress = merge({
			...
			not_erroring: ((newImage.errorCount || 0) <= 10 || (Date.now() - invokeTime) >= 300000) || false,
			not_already_running: (!!newImage.endTime || (invokeTime) + (newImage.maxDuration || newImage.timeout || 300000) < now),
			is_triggered: invokeTime,
			error_count: newImage.errorCount || 0,
			error_timeout: (Date.now() - invokeTime) >= 300000
		}, newImage.progress);
