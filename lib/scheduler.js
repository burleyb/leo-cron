...
module.exports = function (options = {}) {
	...
	let updateQueue = async.queue(async function (trigger, done) {
		...
		if (opts.merge === true) {
			try {
				let res = await dynamodb.merge(CRON_TABLE, id, data, opts); 
				logger.log("== merge res ==", res)
			} catch (err) { 
				if (err) {
					logger.error(id, err)
				}
			...
			try {
				let res = await dynamodb.update(CRON_TABLE, {
					id: id
				}, data, opts)
				
				logger.log("== update res ==", res)
